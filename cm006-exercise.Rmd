...
for some geometric objects, certain aesthetics mappings are required

ggplot(gapminder, x=lifeExp, y=gdpPercap) +geom_point() gives and error
...
|-----------------------|---------------|
| __data__              | `gapminder` |
| __aesthetic mapping__ | `x` and `y` | not quotations, but backticks `
...
+ sign with data wrangling before ggplot. Know when to use piping or +
